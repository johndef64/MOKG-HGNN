...
from utils import accuracy, metrics_calc
...
if __name__ == '__main__':
    ...
    # load input files
    adj, data, label = load_data(args.adjdata, args.featuredata, args.labeldata, mode=0, threshold=args.threshold)
    ...
    if args.mode == 0:
        ...
        for epoch in range(args.epochs):
            ...
            if not os.path.exists('model/GCN'):
                os.makedirs('model/GCN')
        ...
        if os.path.exists('result/GCN_predicted_data.csv'):
            acc, f1 = metrics_calc('data/sample_classes.csv', 'result/GCN_predicted_data.csv')
            print(f'Prediction accuracy on test set: {acc:.4f}, F1-score (weighted): {f1:.4f}')
            #print(f'Prediction accuracy on test set: {accuracy_value:.4f}')
